...
const setCallback = (newCallback) => {
 ...
 const initClient = () => {
  ...
  gapi.client.init({
      apiKey: 'AIzaSyD9Ep9VEWUyv1obeJ-pCQL-D0BdsHs2f3M',
      discoveryDocs: ["https://people.googleapis.com/$discovery/rest?version=v1"],
      clientId: '632465391318-7njr9bgv07f7ia73a2j076j2vedcmoev.apps.googleusercontent.com',
 ...
 const makeApiCall = () => {
  gapi.client.people.people.connections.list({
    ...
    'personFields': 'names,emailAddresses,phoneNumbers,photos'
     ...
     createContacts(response.result.connections);
  }, function(reason) {
    console.log('Error: ' + reason.result.error.message);
  });
}

//callback will be given the contact and should return the value
//to parse
const readFromApiResponse = (contact, property, defaultValue, callback) => {
  if(contact[property] === null || contact[property] === undefined)
    return defaultValue;
  return callback(contact);
}
const loadedContactsFromGoogle = [];
...
const createContacts = (contacts) => {
  contacts.map((contact) => {
    ...
    //not having centralized state is really killing me here.
    parsedContact.id = Math.round(Math.random() * 100000000);
    parsedContact.name = readFromApiResponse(contact, 'names', 'Unknown', (contact) => contact.names[0].displayName);
    parsedContact.phone_number = readFromApiResponse(contact, 'phoneNumbers', '', (contact) => contact.phoneNumbers[0].canonicalForm);
    parsedContact.email = readFromApiResponse(contact, 'emailAddresses', '', (contact) => contact.emailAddresses[0].value);
    parsedContact.image_url = readFromApiResponse(contact, 'photos', '', (contact) => contact.photos[0].url);

    loadedContactsFromGoogle.push(parsedContact);
    }
    catch(error){
      console.log('error while parsing goodle response:', error);
    }
  });

  if(callback) {
    callback(loadedContactsFromGoogle);
  }
}
